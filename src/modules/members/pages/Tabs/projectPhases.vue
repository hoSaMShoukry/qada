<template>
  <div class="container">
   ...
   <div class="text-center" v-if="stages.length == 0"><span class="fs-6 text-danger">لم يتم اضافة مراحل حتى الان</span></div>
   <div v-if="userId != sendFile.projectId" class="text-center"><span class="fs-4 text-danger">غير مسموح الا لمالك المشروع</span></div>

   <div v-if="userId ==sendFile.projectId" >
    <div class="card col-lg-6 col-sm-12 my-5" v-for="stage in stages" :key="stage" >
        ...
        <div class="d-flex justify-content-between align-items-baseline my-2 lastCard">
          <div class="span-stage">
            <span class="d-inline-block pr-5">عدد التعديلات: <span class="numbers"> 2/5 </span></span>
...
export default {
  ...
  setup(){
       ...
       const auth = ref(localStorage.getItem('id'));
       const showModal = ()=>{
  ...
  stages.value =localStorage.getItem('stages') ?  JSON.parse(localStorage.getItem('stages')) : [];
       ...
       return{isModalVisible , stages , showModal , closeModal , sendFile , userId , auth}
  },
}
</script>
