...
<div class="input-group-prepend">
  <span class="input-group-text">سبب الرفض </span>
  <div @click="rejectProject()" class="btn btn-primary">تأكيد العملية</div>
</div>
<textarea class="form-control" aria-label="With textarea" v-model="rejectionReason"></textarea>
</div>
<div class="text-center"><span class="text-danger fs-6">{{ rejected }}</span></div>
...
export default {
  ...
  setup() {
    ...
    'Authorization': 'Bearer ' + localStorage.getItem('token')
      ...
      const acceptProject = async(project)=>{
          ...
          await axios.post(`https://test.m-aboelela.online/api/admin/projects/approve?project_id=${project.id}`,{
  ...
  if(err){
      rejected.value = 'فشل العملية الرجاء التأكد من البيانات'
      setTimeout(() => {
          rejected.value = '';
      }, 3000);
      setTimeout(() => {
