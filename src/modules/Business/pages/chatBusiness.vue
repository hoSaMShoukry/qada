<template>
  <div class="conatiner" style="min-height: 80vh;">
    <div class="row main-chat  mx-auto d-flex my-5 " v-if="chatApi.conversations.length > 0" v-motion-pop style="transition: 0.5s;">
      ...
      <div class="col-4">
        ...
        <div class="conv-menu" style="max-height: 40vh;">
          <div class="d-flex py-2 align-items-center mt-3" v-for="chats , index in chatApi.conversations" :key="index">
            ...
            <div @click="chatApi.getOneConversation('https://test.m-aboelela.online/api/business/chat/conversation_messages',chats.id) , chatApi.activeConversation(index)" 
...
onMounted(async() => {
  ...
  await chatApi.getConversations('https://test.m-aboelela.online/api/business/chat/conversations');
  const channel = pusher.subscribe('private-chat');
     channel.bind('new_message' , (data)=>{
      const obj = { id:Date.now() , is_sender:true , message:data.message }
      console.log(obj);
     chatApi.pushermessage(obj);
    })
});
